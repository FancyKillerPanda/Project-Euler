...
long long int Problem_8()
{
	...
	*/
	...
	std::vector<char> nums_list;
	std::vector<long long int> products;
	std::string target_num = "";


	for (int i = 0; i <= (int) target_num.length() - (target_distance + 1); i++)
	{
		nums_list.clear();

		for (int j = 0; j < (int) target_distance; j++)
		{
			int index = i + j;
			nums_list.push_back(target_num[index]);
		}

		long long int product = 1;

		for (char num : nums_list)
		{
			int int_num = (int)num - 48;
			product *= int_num;
		}
...
///// [PROBLEM #12] - FIRST TRIANGULAR NUMBER TO HAVE OVER 500 DIVISORS /////
...
std::vector<long long int> GetFactors(long long int number)
{
